...
def gameList(request, pk):
    if request.user.is_authenticated:
        ...
        return render(request, 'StatTracker/gameList.html', context={'games': games})
    else:
    ...
    if (request.user.is_authenticated):
        if (pk):
            ...
            if (request.method == 'POST'):
                ...
                gameCreationForm = GameCreationForm(request.POST)
            games = Game.objects.filter(org = org)
            return render(request, 'statTracker/addGames.html', context = {'org': org, 'teams': teams, 'games': games, 'gameCreationForm': gameCreationForm})
        else:
